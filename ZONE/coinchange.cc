...
int coinchangerecur(int arr[],int n,int value)
{
  if(value<=0)
    return 1;
  if(n<=0 && value>0)
    return 0;
  return coinchangerecur(arr,n-1,value)+coinchangerecur(arr,n-1,value-arr[n-1]);
}
...
int main()
{
  ...
  cout << coinchangerecur(arr,n,4) << endl;
  return 0;
}
