...
int dp[2002][2002];
int maxrevenue(int arr[],int start,int end,int count)
{
  if(start>end)
    return 0;
  if(dp[start][end]==0)
    {
      int first = maxrevenue(arr,start+1,end,count+1)+arr[start]*count;
      int last = maxrevenue(arr,start,end-1,count+1)+arr[end]*count;
